...
class Decks {
    ...
    {// implement this method!
    	...
    	try{
    	if (withJokers){
    		originalDecks = new ArrayList<Card>(54 * n);
    		gameDecks = new ArrayList<Card>(54 * n);	
    		int cardIndex = 0;
    			
    		for (int numberOfDecks = 1; numberOfDecks <= n; numberOfDecks++){	
    			for (int numberOfSuite = 0; numberOfSuite <= 4; numberOfSuite++){
    				if (numberOfSuite == 0){
    					originalDecks.add(cardIndex, new Card(0,1));
    					cardIndex++;
    					originalDecks.add(cardIndex, new Card(0,1));
    					cardIndex++;
    				}//end if
    				for (int numberOfRank = 1; numberOfRank <= 13; numberOfRank++){ 
    					if (numberOfSuite != 0){
    					originalDecks.add(cardIndex, new Card(numberOfSuite,numberOfRank));
    					cardIndex++;
    					}//end if
    				}//end for
    			}//end for
    		...
    		originalDecks = new ArrayList<Card>(52 * n);
    		gameDecks = new ArrayList<Card>(52 * n);	
    		int cardIndex = 0;
    		...
    		for (int numberOfDecks = 1; numberOfDecks <= n; numberOfDecks++){	
    			for (int numberOfSuite = 0; numberOfSuite <= 4; numberOfSuite++){
    				if (numberOfSuite == 0){
    					originalDecks.add(cardIndex, new Card(0,1));
    					cardIndex++;
    					originalDecks.add(cardIndex, new Card(0,2));
    					cardIndex++;
    				}//end if
    				for (int numberOfRank = 1; numberOfRank <= 13; numberOfRank++){ 
    					if (numberOfSuite != 0){
    ...
    {// implement this method!
    	...
    	if (numberCards > gameDecks.size()){		//exception handling
    		throw new PlayingCardException ("Not enough cards to deal");
    	}// end if
    	
    	for (int DrawCards = 1; DrawCards <= numberCards; DrawCards++){		//dealing cards, removing them, and adding to cardsDealt list
    		cardsDealt.add(0, gameDecks.remove(gameDecks.size()-1));	
    	}//end for
